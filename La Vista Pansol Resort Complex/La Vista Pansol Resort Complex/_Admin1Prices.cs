...
{
    public partial class _Admin1Prices : Form
    {
        public _Admin1Prices()
        {
            InitializeComponent();
        }
        ...
        public void _Admin1Rooms_Load(object sender, EventArgs e)
        {
            accomodation = new MySqlConnection("server=" + hostName + "; user=root; password= ; database=accomodation;");
            userdb = new MySqlConnection("server=" + hostName + "; user=root; password= ; database=login_info;");
        ...
        private void cmbCottages_SelectedIndexChanged(object sender, EventArgs e)
        {
            _selectCottages();
        }

        private void cmbPrivatePools_SelectedIndexChanged(object sender, EventArgs e)
        {
            _packageType();
            _selectPrivatePools();
        }
        ...
        private void cmbRentals_SelectedIndexChanged(object sender, EventArgs e)
        {
            _selectRentals();
        }

        private void cmbPoolAccess_SelectedIndexChanged(object sender, EventArgs e)
        {
            _selectPoolAccess();
        }
        ...
        {
            if(tbUsername.Text == username && tbPassword.Text == password && tbEmployeeID.Text == employeeID)
            {
                bool update = false;
                if (tbRoomPax.Text.Length > 0 && tbRoomPrice12.Text.Length > 0 && tbRoomPrice22.Text.Length > 0 && tbRoomUnits.Text.Length > 0)
                {
                    update = true;
                    _updateRooms();
                }
                if (tbCottagesPax.Text.Length > 0 && tbCottagesPriceAM.Text.Length > 0 && tbCottagesPricePM.Text.Length > 0 && tbCottagesUnits.Text.Length > 0)
                {
                    update = true;
                    _updateCottages();
                }
                if (tbPrivatePoolsPax.Text.Length > 0 && tbPrivatePoolsPrice.Text.Length > 0 && tbPrivatePoolsUnits.Text.Length > 0)
                {
                    update = true;
                    _updatePrivatePools();
                }
                if (tbHallsPax.Text.Length > 0 && tbHallsPrice.Text.Length > 0 && tbHallsUnits.Text.Length > 0)
                {
                    update = true;
                    _updateHalls();
                }
                if (tbRentalsHour.Text.Length > 0 && tbRentalsPriceAM.Text.Length > 0 && tbRentalsPricePM.Text.Length > 0)
                {
                    update = true;
                    _updateRentals();
                }
                if (tbAccessAdultAM.Text.Length > 0 && tbAccessAdultPM.Text.Length > 0 && tbAccessKidsAM.Text.Length > 0 && tbAccessKidsPM.Text.Length > 0)
                {
                    update = true;
                    _updatePoolAccess();
                }
                if(update == true)
                {
                    MessageBox.Show("Your assets are successfully updated!");
                }
                else
                {
                    MessageBox.Show("There is no assets to be updated!");
                }
                _reset();
            }
            else
            {
                MessageBox.Show("Please verify your information before to continue.");
            }
        }
        ...
        public void _updateRooms()
        {
            ...
            try
            {
                ...
                MySqlCommand mySqlCommand = accomodation.CreateCommand();
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                mySqlCommand.CommandText = "UPDATE info_rooms SET paxRange = '" + tbRoomPax.Text + "'," +
                    " priceAt12 = '" + tbRoomPrice12.Text + "', priceAt22 = '" + tbRoomPrice22.Text + "' WHERE ID='" + count + "'";
                mySqlCommand.ExecuteNonQuery();
                accomodation.Close();
            }
            catch (Exception exc)
        ...
        public void _updateCottages()
        {
            ...
            try
            {
                ...
                MySqlCommand mySqlCommand = accomodation.CreateCommand();
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                mySqlCommand.CommandText = "UPDATE info_cottages SET paxRange = '" + tbCottagesPax.Text + "'," +
                    " amPrice = '" + tbCottagesPriceAM.Text + "', pmPrice = '" + tbCottagesPricePM.Text + "' WHERE ID='" + count + "'";
                mySqlCommand.ExecuteNonQuery();
                accomodation.Close();
            }
            catch (Exception exc)
        ...
        private void tbRoomUnits_KeyPress(object sender, KeyPressEventArgs e)
        {
            Char num = e.KeyChar;
        ...
        public void _updatePrivatePools()
        {
            int count = 1;
            ...
            try
            {
                ...
                MySqlCommand mySqlCommand = accomodation.CreateCommand();
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                mySqlCommand.CommandText = "UPDATE info_privatepools SET paxRange = '" + tbPrivatePoolsPax.Text + "'," +
                    " packageA = '" + packageA+ "', packageB = '" + packageB + "', packageC = '" + packageC + "' WHERE ID='" + count + "'";
                mySqlCommand.ExecuteNonQuery();
                accomodation.Close();
            }
            catch (Exception exc)
        ...
        public void _updateHalls()
        {
            ...
            try
            {
                ...
                MySqlCommand mySqlCommand = accomodation.CreateCommand();
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                mySqlCommand.CommandText = "UPDATE info_halls SET paxRange = '" + tbHallsPax.Text + "'," +
                    " roomPrice = '" + tbCottagesPriceAM.Text + "' WHERE ID='" + count + "'";
                mySqlCommand.ExecuteNonQuery();
                accomodation.Close();
            }
            catch (Exception exc)
        ...
        public void _updateRentals()
        {
            ...
            try
            {
                ...
                MySqlCommand mySqlCommand = accomodation.CreateCommand();
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                mySqlCommand.CommandText = "UPDATE info_rentals SET amPrice = '" + tbRentalsPriceAM.Text + "'," +
                    " pmPrice = '" + tbRentalsPricePM.Text + "' WHERE ID='" + count + "'";
                mySqlCommand.ExecuteNonQuery();
                accomodation.Close();
            }
            catch (Exception exc)
        ...
        public void _updatePoolAccess()
        {
            ...
            try
            {
                ...
                MySqlCommand mySqlCommand = accomodation.CreateCommand();
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                mySqlCommand.CommandText = "UPDATE info_entrance SET adultAM = '" + tbAccessAdultAM.Text + "', adultPM = '" + tbAccessAdultPM.Text + "'," +
                    " kidsAM = '" + tbAccessKidsAM.Text + "', kidsPM = '" + tbAccessKidsPM.Text + "' WHERE ID='" + count + "'";
                mySqlCommand.ExecuteNonQuery();
                accomodation.Close();
            }
            catch (Exception exc)
